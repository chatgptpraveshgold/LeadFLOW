...
const CACHE_NAME = 'pravesh-crm-v1';
const RUNTIME_CACHE = 'pravesh-runtime-v1';
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(
    caches.match(event.request)
      ...
      .catch(() => caches.match('/leadflow.html'))
  );
});
...
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Pravesh Gold', {
      body: data.body,
